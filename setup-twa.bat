...
if %errorlevel% neq 0 (
    ...
    for /f "tokens=* delims=" %%i in ('java -version 2^>^&1 ^| findstr version') do echo  ✓  %%i
)
...
if %errorlevel% neq 0 (
    ...
    if %errorlevel% neq 0 (
        echo  ✗  Bubblewrap 설치 실패. npm 권한을 확인하세요.
        pause & exit /b 1
    )
    echo  ✓  Bubblewrap CLI 설치 완료
) else (
    for /f %%i in ('bubblewrap --version') do echo  ✓  Bubblewrap %%i (이미 설치됨)
)
